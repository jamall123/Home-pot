    ...
    return Scaffold(
      body: Consumer<StoreProvider>(
        builder: (context, provider, child) {
          final products = provider.getByCategory(ProductCategory.canned);
          if (products.isEmpty) {
            return const Center(
              child: Text(
                'لا توجد منتجات معلبة/كراتين.\nاضغط على + للإضافة',
                ...
                child: Card(
                  child: ListTile(
                    onTap: () => showModalBottomSheet(
                      ...
                      builder: (ctx) => AddReadyProductSheet(product: product),
                    ),
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text('كرتونة: ${product.quantity.toStringAsFixed(0)} قطعة'),
                        Text('شراء الكرتونة: ${product.purchasePrice.toStringAsFixed(1)} ${provider.currency}'),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => showModalBottomSheet(
          context: context,
          isScrollControlled: true,
          builder: (ctx) => const AddReadyProductSheet(),
        ),
...
class AddReadyProductSheet extends StatefulWidget {
  final Product? product;
  const AddReadyProductSheet({super.key, this.product});

  @override
  State<AddReadyProductSheet> createState() => _AddReadyProductSheetState();
}

class _AddReadyProductSheetState extends State<AddReadyProductSheet> {
  final _formKey = GlobalKey<FormState>();
  final _nameController = TextEditingController();
  final _purchasePriceController = TextEditingController();
  final _quantityController = TextEditingController(); // Items in carton
  ...
  // Helper to parse double from formatted string (removes commas)
    ...
    return Padding(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            const Text('إضافة منتج معلب/كرتونة', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              children: [
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    keyboardType: TextInputType.number,
                    inputFormatters: [ThousandsSeparatorInputFormatter()],
                    decoration: const InputDecoration(labelText: 'عدد القطع', border: OutlineInputBorder()),
                    onChanged: (_) => _calculateCost(),
                    validator: (val) => val!.isEmpty ? 'مطلوب' : null,
                  ),
                ),
