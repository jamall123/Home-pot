    ...
    return Scaffold(
      body: Consumer<StoreProvider>(
        builder: (context, provider, child) {
          final products = provider.getByCategory(ProductCategory.beverages);
          if (products.isEmpty) {
            return const Center(
              child: Text(
                'لا توجد مشروبات/ألبان.\nاضغط على + للإضافة',
                ...
                child: Card(
                  child: ListTile(
                    onTap: () => showModalBottomSheet(
                      ...
                      builder: (ctx) => AddBeverageProductSheet(product: product),
                    ),
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text('عبوة دبل/بدتة: ${product.quantity.toStringAsFixed(0)} قطعة'),
                        Text('شراء العبوة: ${product.purchasePrice.toStringAsFixed(1)} ${provider.currency}'),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => showModalBottomSheet(
          context: context,
          isScrollControlled: true,
          builder: (ctx) => const AddBeverageProductSheet(),
        ),
...
class AddBeverageProductSheet extends StatefulWidget {
  final Product? product;
  const AddBeverageProductSheet({super.key, this.product});

  @override
  State<AddBeverageProductSheet> createState() => _AddBeverageProductSheetState();
}

class _AddBeverageProductSheetState extends State<AddBeverageProductSheet> {
    ...
    return Padding(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            const Text('إضافة مشروبات/ألبان', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'سعر شراء العبوة/الدبل', border: OutlineInputBorder()),
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    keyboardType: TextInputType.number,
                    inputFormatters: [ThousandsSeparatorInputFormatter()],
                    decoration: const InputDecoration(labelText: 'عدد القطع في العبوة', border: OutlineInputBorder()),
                    onChanged: (_) => _calculateCost(),
                    validator: (val) => val!.isEmpty ? 'مطلوب' : null,
                  ),
                ),
